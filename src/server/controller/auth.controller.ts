...
const challengeSchema = z.object({ publicKey: publicKeySchema });
...
export async function requestChallenge(req: NextRequest) {
  ...
  const { xdr, nonce, expiresAt } = await authService.createChallenge(publicKey);
  return ok({ xdr, nonce, expiresAt: expiresAt.toISOString() });
}

export async function verifyChallenge(req: NextRequest) {
  const { publicKey, signedXdr } = verifySchema.parse(await req.json());
  const { sessionId } = await authService.verifyAndCreateSession(publicKey, signedXdr);
...
export async function logout(req: NextRequest) {
  ...
  if (sessionId) await authService.destroySession(sessionId);
